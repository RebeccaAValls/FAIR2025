...
knitr::opts_knit$set(root.dir = "~/Dartmouth College Dropbox/Rebecca Valls/Classes_Workshops/FAIR/FAIR 2025/MicrobiomeDay2") #This line sets the root directory for the R Markdown file. 
```
...
#install for macs
#devtools::install_github("pmartinezarbizu/pairwiseAdonispairwiseAdonis")
...
count_df <- as.data.frame(read.table("~/Dartmouth College Dropbox/Rebecca Valls/Classes_Workshops/FAIR/FAIR 2025/MicrobiomeDay2/July08_2025_MicrobiomeAnalysisPart2_counts.tsv", header=T, row.names=1, check.names=F, sep="\t"))
...
tax_df <- as.matrix(read.table("~/Dartmouth College Dropbox/Rebecca Valls/Classes_Workshops/FAIR/FAIR 2025/MicrobiomeDay2/July08_2025_MicrobiomeAnalysisPart2_taxa.tsv", header=T, row.names=1, check.names=F, sep="\t"))
...
metadata <- metadata %>% remove_rownames %>% column_to_rownames(var="SeqCoastTube")

#Factor order of sample info in a way that makes sense.
metadata$Timepoint <- factor(metadata$Timepoint, levels = c("Baseline_3", "Baseline_6", "Antibiotics_3", "Antibiotics_6", "Gavage_1", "Gavage_4", "Before_LPS", "LPS", "Sacrifice" ))

metadata$Treatment <- factor(metadata$Treatment, levels = c("Baseline", "PBS", "Bacteroides"))
...
```{r}
metadata %>%
  mutate(sample_or_control = if_else(Treatment == "Baseline","control","sample")) -> metadata # add sample_or_control column to identify samples for decontam
...
plot_frequency(Phyloseq_object_NZ, 
               taxa_names(Phyloseq_object_NZ)[c(1,15997)], 
               conc="DNA_ng.uL") + 
...
# Run PERMANOVA using adonis2
before_LPS_perm_results <- adonis2(distance_bray_Before_LPS ~ Treatment + Sex, data = ord_Before_LPS_df, permutations = 999, by="terms") #see what happens when you add by="terms" to the adonis2 command
...
bacteroides_relabundance_lm <- lmerTest::lmer(Abundance ~ Treatment + Sex + (1|Cage), data = bacteroides_rel_ab_LPS)
...
#Let's train a random forest model on whether or not a mouse belongs to the Treatment group PBS or Bacteroides
```{r}
set.seed(42)
